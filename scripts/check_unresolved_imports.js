...
function resolveImport(file, imp){
  ...
  const candidates = [];
  const p = path.resolve(dir, imp);
  candidates.push(p);
  for(const e of exts) candidates.push(p+e);
  for(const e of exts) candidates.push(path.join(p,'index'+e));
...
fs.writeFileSync('/tmp/unresolved-imports.json', JSON.stringify(out,null,2));
console.log('WROTE /tmp/unresolved-imports.json');
